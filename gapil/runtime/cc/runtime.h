...
typedef struct gapil_pool_t gapil_pool;
typedef struct gapil_globals_t gapil_globals;
...
// gapil_context contains information about the environment in which a function
// is executing.
typedef struct gapil_context_t {
  uint32_t id;  // the context identifier. Can be treated as user-data.
  gapil_globals* globals;  // a pointer to the global state.
...
// gapil_pool describes the underlying buffer that may be used by one or more
// slices.
typedef struct gapil_pool_t {
  uint32_t ref_count;  // number of owners of this pool.
  uint32_t id;         // the unique identifier for the pool.
} gapil_pool;

// gapil_slice is the data of a gapil slice type (elty foo[]).
typedef struct gapil_slice_t {
...
typedef struct gapil_module_t {
  ...
  void (*destroy_context)(gapil_context*);
...
typedef struct gapil_runtime_callbacks_t {
  ...
  void (*call_extern)(gapil_context*, uint8_t* name, void* args, void* res);

  // copies N bytes of data from src to dst, where N is min(dst.size, src.size).
  void (*copy_slice)(gapil_context*, gapil_slice* dst, gapil_slice* src);
  ...
  void (*cstring_to_slice)(gapil_context*, uint64_t ptr, gapil_slice* out);
  ...
  void (*store_in_database)(gapil_context* ctx, void* ptr, uint64_t size,
                            uint8_t* id_out);

  // allocates and returns a new pool with the given size in bytes and an
  // initial reference count of 1.
  gapil_pool* (*make_pool)(gapil_context*, uint64_t size);

  // frees a pool previously allocated with gapil_make_pool or
  // gapil_string_to_slice.
  void (*free_pool)(gapil_pool*);
...
DECL_GAPIL_CB(void, gapil_string_reference, gapil_string*);
DECL_GAPIL_CB(void, gapil_string_release, gapil_string*);

DECL_GAPIL_CB(void, gapil_slice_reference, gapil_slice*);
DECL_GAPIL_CB(void, gapil_slice_release, gapil_slice*);

// throws an exception to immediately terminate exeuction of the currently
// executing command.
DECL_GAPIL_CB(void, gapil_abort);
...
DECL_GAPIL_CB(void, gapil_create_buffer, arena*, uint64_t capacity,
              uint64_t alignment, gapil_buffer*);

// destroys a buffer previously created with gapil_create_buffer.
DECL_GAPIL_CB(void, gapil_destroy_buffer, gapil_buffer*);

// appends data to a buffer.
DECL_GAPIL_CB(void, gapil_append_buffer, gapil_buffer*, const void* data,
              uint64_t size);

// returns a pointer to the underlying buffer data for the given slice,
// using gapil_data_resolver if it has been set.
DECL_GAPIL_CB(void*, gapil_slice_data, gapil_context*, gapil_slice*,
              gapil_data_access);

// copies N bytes of data from src to dst, where N is min(dst.size, src.size).
DECL_GAPIL_CB(void, gapil_copy_slice, gapil_context*, gapil_slice* dst,
              gapil_slice* src);

// allocates a new slice and underlying pool filled with the data of string.
DECL_GAPIL_CB(void, gapil_string_to_slice, gapil_context*, gapil_string* string,
              gapil_slice* out);

// allocates a new string with the given data and length.
// length excludes a null-pointer.
DECL_GAPIL_CB(gapil_string*, gapil_make_string, arena*, uint64_t length,
              void* data);
...
DECL_GAPIL_CB(void, gapil_cstring_to_slice, gapil_context*, uint64_t ptr,
              gapil_slice* out);

// frees a string allocated with gapil_make_string, gapil_string_concat or
// gapil_slice_to_string.
DECL_GAPIL_CB(void, gapil_free_string, gapil_string*);

// allocates a new string filled with the data of slice.
DECL_GAPIL_CB(gapil_string*, gapil_slice_to_string, gapil_context*,
              gapil_slice* slice);

// allocates a new string containing the concatenated data of the two strings.
DECL_GAPIL_CB(gapil_string*, gapil_string_concat, gapil_string*, gapil_string*);

// compares two strings lexicographically, using the same rules as strcmp.
DECL_GAPIL_CB(int32_t, gapil_string_compare, gapil_string*, gapil_string*);
...
// applys the read observations tagged to the current command into the memory
// model.
DECL_GAPIL_CB(void, gapil_apply_reads, gapil_context*);

// applys the write observations tagged to the current command into the memory
// model.
DECL_GAPIL_CB(void, gapil_apply_writes, gapil_context*);

// Returns a pointer to the pool's data starting at pointer for size bytes.
DECL_GAPIL_CB(void*, gapil_resolve_pool_data, gapil_context*, gapil_pool*,
              uint64_t ptr, gapil_data_access, uint64_t size);

// stores the buffer at ptr of the given size into the database.
// Writes the 20-byte database identifier of the stored data to id.
DECL_GAPIL_CB(void, gapil_store_in_database, gapil_context* ctx, void* ptr,
              uint64_t size, uint8_t* id_out);

// allocates and returns a new pool with the given size in bytes and an
// initial reference count of 1.
DECL_GAPIL_CB(gapil_pool*, gapil_make_pool, gapil_context*, uint64_t size);

// frees a pool previously allocated with gapil_make_pool or
// gapil_string_to_slice.
DECL_GAPIL_CB(void, gapil_free_pool, gapil_pool*);

// calls the extern with the given name and arguments. If the extern has a
// return type, then the result should be be stored to res.
DECL_GAPIL_CB(void, gapil_call_extern, gapil_context*, uint8_t* name,
              void* args, void* res);
