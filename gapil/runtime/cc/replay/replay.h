...
typedef struct gapil_replay_data_t {
  // instructions currently being built.
  gapil_buffer instructions;
...
typedef struct gapil_replay_payload_t {
  // Maximum stack size in elements for replay.
  uint32_t stack_size;

  // Maximum size of the volatile memory in bytes.
  uint32_t volatile_size;

  // The replay opcodes.
  gapil_buffer opcodes;

  // buffer of gapil_replay_resource_info representing all the resources used by
  // the replay.
  gapil_buffer resources;

  // The replay constant data.
  gapil_buffer constants;

} gapil_replay_payload;
////////////////////////////////////////////////////////////////////////////////
// Runtime API implemented in replay.cpp                                      //
////////////////////////////////////////////////////////////////////////////////

// gapil_replay_build builds the replay payload used by GAPIR.
// Call gapil_replay_release_payload to free payload memory.
void gapil_replay_build(gapil_context* ctx, gapil_replay_data* data,
                        gapil_replay_payload* payload);

// gapil_replay_free_payload frees the memory used by the payload.
...
// gapil_replay_add_resource_by_slice is called whenever a slice needs to
// be encoded as a resource. The resource identifier is returned.
DECL_GAPIL_REPLAY_FUNC(uint32_t, gapil_replay_add_resource_by_slice,
                       gapil_context* ctx, gapil_replay_data* data,
                       gapil_slice* slice);

// gapil_replay_add_resource_by_id is called whenever a database entry needs to
// be encoded as a resource. id is a 20-byte database key identifier. The
// resource identifier is returned.
DECL_GAPIL_REPLAY_FUNC(uint32_t, gapil_replay_add_resource_by_id,
                       gapil_context* ctx, gapil_replay_data* data, uint8_t* id,
                       uint64_t size);
