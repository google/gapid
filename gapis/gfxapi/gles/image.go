...
func getSizedFormatFromTuple(unsizedFormat, ty GLenum) (sizedFormat GLenum) {
	sf, _ := subGetSizedFormatFromTuple(nil, nil, nil, &gfxapi.State{}, nil, 0, nil, unsizedFormat, ty)
...
func getUnsizedFormatAndType(sizedFormat GLenum) (unsizedFormat, ty GLenum) {
	info, _ := subGetSizedFormatInfo(nil, nil, nil, &gfxapi.State{}, nil, 0, nil, sizedFormat)
...
func getUncompressedStreamFormat(unsizedFormat, ty GLenum) (format *stream.Format, err error) {
	info, _ := subGetUnsizedFormatInfo(nil, nil, nil, &gfxapi.State{}, nil, 0, nil, unsizedFormat)
	if info.Count == 0 {
