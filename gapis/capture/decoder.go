...
func (d *decoder) decode(ctx context.Context, in proto.Message) (interface{}, error) {
	...
	case *Resource:
		index, err := d.builder.addRes(ctx, obj.Data)
		if err != nil {
			return nil, err
		}
		// If the Resource has the optional Index field, use it for verification.
		if obj.Index != 0 && index != obj.Index {
			panic(fmt.Errorf("Resource has index %v but we expected %v", index, obj.Index))
		}
		return in, nil
