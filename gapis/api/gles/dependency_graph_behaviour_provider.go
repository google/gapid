		...
		if isEglSwapBuffers {
			...
			case *GlCopyImageSubData:
				...
				if cmd.SrcTarget == GLenum_GL_RENDERBUFFER {
					b.Read(g, renderbufferDataKey{c.Objects.Renderbuffers[RenderbufferId(cmd.SrcName)]})
				} else {
					data, size := c.Objects.Textures[TextureId(cmd.SrcName)].dataAndSize(cmd.SrcLevel, 0)
					b.Read(g, data)
					b.Read(g, size)
				}
				if cmd.DstTarget == GLenum_GL_RENDERBUFFER {
					b.Write(g,
						renderbufferDataKey{c.Objects.Renderbuffers[RenderbufferId(cmd.DstName)]})
				} else {
					data, size := c.Objects.Textures[TextureId(cmd.DstName)].dataAndSize(cmd.DstLevel, 0)
					b.Write(g, data)
					b.Write(g, size)
				}
			case *GlFramebufferTexture2D:
				b.Read(g, textureSizeKey{c.Objects.Textures[cmd.Texture], cmd.Texture, cmd.Level, 0})
