...
func (a API) Replay(
	...
	for _, rr := range rrs {
		...
		case profileRequest:
			...
			optimize = false
			transforms.Add(newSliceCommandMapper(uint64(numInitialCommands), req.submissionIds))
...
func (a API) Profile(
	...
	d, err := trace.ProcessProfilingData(ctx, intent.Device, &buffer, &handleMappings, &submissionIds)
	return d, err
}
