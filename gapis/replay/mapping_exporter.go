...
type MappingExporter struct {
	mappings []*service.VulkanHandleMappingItem
	thread   uint64
	path     string
}

func NewMappingExporter(ctx context.Context) *MappingExporter {
	return &MappingExporter{
		mappings: make([]*service.VulkanHandleMappingItem, 0, 0),
		thread:   0,
		path:     "",
	}
}
...
func (m *MappingExporter) GetMappings() []*service.VulkanHandleMappingItem {
	return m.mappings
}
	...
	for k, v := range b.Remappings {
		...
		func(k interface{}, typ reflect.Type, size uint64) {
			b.Post(v, size, func(r binary.Reader, err error) {
				...
				m.mappings = append(m.mappings, &service.VulkanHandleMappingItem{HandleType: typ.Name(), TraceValue: fmt.Sprintf("%v", k), ReplayValue: fmt.Sprintf("%v", val)})
			})
		}(k, typ, size)
	}
...
func printToFile(ctx context.Context, path string, mappings []*service.VulkanHandleMappingItem) {
