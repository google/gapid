...
func (t *androidTracer) ProcessProfilingData(ctx context.Context, buffer *bytes.Buffer, handleMappings *map[uint64][]service.VulkanHandleMappingItem, submissionIds *[]uint64) (*service.ProfilingData, error) {
	...
	if strings.Contains(gpuName, "Adreno") {
		return adreno.ProcessProfilingData(ctx, processor, desc, handleMappings, submissionIds)
	}
