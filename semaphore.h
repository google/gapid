...
struct VkSemaphoreWrapper : handle_base<VkSemaphore> {
  VkSemaphoreWrapper(HandleUpdater*, VkDevice, VkSemaphore semaphore)
      : handle_base<VkSemaphore>(semaphore) {}

  void set_create_info(const VkSemaphoreCreateInfo* pCreateInfo) {
    create_info = mem.get_typed_memory<VkSemaphoreCreateInfo>(1);
