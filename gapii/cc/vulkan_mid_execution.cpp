...
void VulkanSpy::EnumerateVulkanResources(CallObserver* observer) {
    for(auto& instance: Instances) {
        ...
        create_info.menabledExtensionCount = instance.second->mEnabledExtensions.size();

        const char** enabled_layers = (const char**)alloca(sizeof(char*) * create_info.menabledLayerCount);
        const char** enabled_extensions = (const char**)alloca(sizeof(char*) * create_info.menabledExtensionCount);
        ...
        create_info.mppEnabledLayerNames = create_info.menabledLayerCount > 0 ? (char**)enabled_layers: nullptr;
        create_info.mppEnabledExtensionNames = create_info.menabledExtensionCount > 0 ? (char**)enabled_extensions:  nullptr;
        ...
        for (auto& surface: Surfaces) {
            switch(surface.second->mType) {
                case SurfaceType::SURFACE_TYPE_XCB: {
                    ...
                    RecreateXCBSurfaceKHR(observer, surface.second->mInstance, &create_info,
    ...
    std::map<VkInstance, std::vector<VkPhysicalDevice>> devices;
     for (auto& physical_device: PhysicalDevices) {
         ...
         if (it == devices.end()) {
             it = devices.insert(std::make_pair(physical_device.second->mInstance,
                 std::vector<VkPhysicalDevice>())).first;
         }
     ...
     for (auto& instance_devices: devices) {
         ...
         RecreatePhysicalDevices(observer, instance_devices.first,
             &count, instance_devices.second.data());
     }
     for (auto& physical_device: PhysicalDevices) {
         ...
         mImports.mVkInstanceFunctions[physical_device.second->mInstance].
            vkGetPhysicalDeviceQueueFamilyProperties(
                    physical_device.second->mVulkanHandle,
         ...
         RecreatePhysicalDeviceProperties(observer, physical_device.second->mVulkanHandle,
            &queueFamilyPropertyCount, queueFamilyProperties.data(), &memory_properties);
     }
    }
    for (auto& device: Devices) {
        mImports.mVkDeviceFunctions[device.second->mVulkanHandle].vkDeviceWaitIdle(device.second->mVulkanHandle);
        ...
        create_info.menabledExtensionCount = device.second->mEnabledExtensions.size();
        ...
        for(auto queue: device.second->mQueues) {
            ...
            auto a = std::find_if(queue_create_infos.begin(),
             queue_create_infos.end(), [&](VkDeviceQueueCreateInfo& a) {
            ...
            queue_priorities[a - queue_create_infos.begin()][queue_index] = queue_priority;
        }
    ...
    }
    {
        ...
        create_info.msType = VkStructureType::VK_STRUCTURE_TYPE_SWAPCHAIN_CREATE_INFO_KHR;
        for(auto& swapchain: Swapchains) {
            ...
            std::vector<uint32_t> imageLayouts(swapchainImages, VkImageLayout::VK_IMAGE_LAYOUT_UNDEFINED);
            std::vector<VkQueue> lastQueues(swapchainImages, 0);
            mImports.mVkDeviceFunctions[device].vkGetSwapchainImagesKHR(
                device, swapchain.second->mVulkanHandle, &swapchainImages, images.data());
    ...
    // Bind and Fill the "recreated" buffer memory
    {
        ...
        for(auto& buffer: Buffers) {
            ...
            std::shared_ptr<DeviceObject> device_object = Devices[buffer.second->mDevice];
            VkPhysicalDevice& physical_device = device_object->mPhysicalDevice;
            auto& device_functions = mImports.mVkDeviceFunctions[buffer.second->mDevice];
    ...
    // Bind and Fill the "recreated" image memory
    {
        ...
        for(auto& image: Images) {
            ...
            std::shared_ptr<DeviceObject> device_object = Devices[image.second->mDevice];
            VkPhysicalDevice& physical_device = device_object->mPhysicalDevice;
            auto& device_functions = mImports.mVkDeviceFunctions[image.second->mDevice];
            VkInstance& instance = PhysicalDevices[physical_device]->mInstance;
            ...
            if (image.second->mBoundMemory &&
                ...
                memory_barrier.msrcAccessMask = VkAccessFlagBits::VK_ACCESS_TRANSFER_READ_BIT;
                memory_barrier.mdstAccessMask = (VkAccessFlagBits::VK_ACCESS_MEMORY_WRITE_BIT << 1) - 1;
                memory_barrier.moldLayout = VkImageLayout::VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL;
...

    {
        ...
        for (auto& fence_object: Fences) {
            ...
            if (status == VkResult::VK_SUCCESS) {
                create_info.mflags = VkFenceCreateFlagBits::VK_FENCE_CREATE_SIGNALED_BIT;
            } else {
                create_info.mflags = 0;
            }
    ...
    }
    {
        ...
        for (auto& commandPool: CommandPools) {
            ...
            RecreateCommandPool(observer,
                commandPool.second->mDevice,
                &create_info,
...

    {
        ...
        for (auto& descriptorSetLayout: DescriptorSetLayouts) {
            ...
            for (auto& binding: descriptorSetLayout.second->mBindings) {
                bindings.push_back({
                    binding.first,
                    binding.second.mType,
                    binding.second.mCount,
                    binding.second.mStages,
                    nullptr
                });
                immutableSamplers.push_back({});
                if (binding.second.mImmutableSamplers.size()) {
                    for(size_t i = 0; i < binding.second.mImmutableSamplers.size(); ++i) {
                        immutableSamplers.back().push_back(binding.second.mImmutableSamplers[i]->mVulkanHandle);
                    }
                }
            }
            ...
            RecreateDescriptorSetLayout(observer, descriptorSetLayout.second->mDevice,
                &create_info, &descriptorSetLayout.second->mVulkanHandle);
        }
    }

    {
        ...
        for (auto& pipelineLayout: PipelineLayouts) {
            ...
            create_info. mpushConstantRangeCount = pipelineLayout.second->mPushConstantRanges.size();
            ...
            RecreatePipelineLayout(observer, pipelineLayout.second->mDevice,
                &create_info, &pipelineLayout.second->mVulkanHandle);
        }
    }
    {
    ...
    }
    {
        ...
        for (auto& pool: DescriptorPools) {
            ...
            RecreateDescriptorPool(observer, pool.second->mDevice,
                &create_info, &pool.second->mVulkanHandle);
        }
    }
    {
        ...
        for (auto& descriptorSet: DescriptorSets) {
            ...
            RecreateDescriptorSet(observer,
                descriptorSet.second->mDevice,
                &allocate_info,
                writes.size(),
                writes.data(),
    ...
    }
    {
        ...
        create_info.msType = VkStructureType::VK_STRUCTURE_TYPE_FRAMEBUFFER_CREATE_INFO;
        for (auto& framebuffer: Framebuffers) {
            std::vector<VkImageView> imageAttachments(framebuffer.second->mImageAttachments.size());
            for (size_t i = 0; i < imageAttachments.size(); ++i) {
                imageAttachments[i] = framebuffer.second->mImageAttachments[i]->mVulkanHandle;
            }
            create_info.mrenderPass = framebuffer.second->mRenderPass->mVulkanHandle;
            create_info.mattachmentCount = framebuffer.second->mImageAttachments.size();
