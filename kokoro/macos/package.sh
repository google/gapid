...
# This path to dmgbuild (installed via pip --user) may change with Kokoro MacOS
# updates. Value of 'site.USER_BASE' can give a hint of where to look, but
# beware that default Python version may be different from the pip one.
/Users/kbuilder/Library/Python/3.7/bin/dmgbuild -s dmg-settings.py GAPID gapid-$VERSION-macos.dmg
