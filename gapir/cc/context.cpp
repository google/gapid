...
std::unique_ptr<Context> Context::create(
    ...
    core::CrashHandler& crash_handler, ResourceProvider* resource_provider,
    MemoryManager* memory_manager) {
  std::unique_ptr<Context> context(
      new Context(conn, crash_handler, resource_provider, memory_manager));
  GAPID_INFO("context pointer created");

  if (context->initialize()) {
    GAPID_INFO("context initialized succeeded");
    return context;
  } else {
    GAPID_INFO("context initialized failed");
    return nullptr;
  }
}
...
Context::Context(ReplayConnection* conn, core::CrashHandler& crash_handler,
    ...
    : mConnection(conn),
      ...
      mPostBuffer(new PostBuffer(
          POST_BUFFER_SIZE, [this](std::unique_ptr<ReplayConnection::Posts> posts) {
...
bool Context::initialize() {
  ...
  if (mReplayRequest == nullptr) {
    GAPID_INFO("replay request create failed");
    return false;
  }
  GAPID_INFO("replay request create succeeded");
  ...
  GAPID_INFO("replay request set volatile memory");
  ...
  interpreter->registerBuiltin(
      ...
      [this](uint32_t label, Stack* stack, bool) {
        ...
        if (stack->isValid()) {
          ...
          if (renderer == nullptr) {
            GAPID_WARNING(
                "[%u]replayChangeBackbuffer called with unknown renderer %d",
                label, renderer);
            return false;
          }
