...
bool MemoryManager::setReplayDataSize(uint32_t constantMemorySize,
  ...
  GAPID_INFO("in MemoryManager::setReplayDataSize(%d, %d)", constantMemorySize, opcodeMemorySize);
  if (opcodeMemorySize > mSize) {
    GAPID_INFO("in setReplayDataSize, opcodeMemorySize > mSize");
    return false;
  }
  GAPID_INFO("in setReplayDataSize, opcodeMemorySize <= mSize");
  ...
  if (constantMemorySize > mSize - mOpcodeMemory.size) {
    GAPID_INFO("in setReplayDataSize, constant memory size too large");
    return false;
  }
  mConstantMemory = {align(mOpcodeMemory.base - constantMemorySize),
                     constantMemorySize};
  GAPID_INFO("Constant range: [%p,%p]", mConstantMemory.base,
  ...
  GAPID_INFO("Replay range: [%p,%p]", mReplayData.base,
