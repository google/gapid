...
struct VkFramebufferWrapper : handle_base<VkFramebuffer> {
  VkFramebufferWrapper(HandleUpdater*, VkDevice, VkFramebuffer framebuffer)
      : handle_base<VkFramebuffer>(framebuffer) {}

  void set_create_info(const VkFramebufferCreateInfo* pCreateInfo) {
    create_info = mem.get_typed_memory<VkFramebufferCreateInfo>(1);
    clone<NullCloner>(&cloner, pCreateInfo[0], create_info[0], &mem,
                      _VkFramebufferCreateInfo_pAttachments_valid);
  }
