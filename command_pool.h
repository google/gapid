...
struct VkCommandPoolWrapper : handle_base<VkCommandPool> {
  VkCommandPoolWrapper(HandleUpdater*, VkDevice, VkCommandPool command_pool)
      : handle_base<VkCommandPool>(command_pool) {}

  void set_create_info(const VkCommandPoolCreateInfo* pCreateInfo) {
    create_info = mem.get_typed_memory<VkCommandPoolCreateInfo>(1);
