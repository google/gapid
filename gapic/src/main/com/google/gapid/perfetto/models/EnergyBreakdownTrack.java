...
import com.google.gapid.perfetto.views.EnergyBreakdownSelectionView;
...
public class EnergyBreakdownTrack extends Track.WithQueryEngine<EnergyBreakdownTrack.Data> {
  private static final String VIEW_SQL_MONOTONIC =
      "select ts + 1 ts, lead(ts) over win - ts dur, lead(value) over win - value value, lead(id)"
          + " over win id from counter where track_id = %d window win as (order by ts)";
  private static final String SUMMARY_SQL =
      "select min(ts), max(ts + dur), avg(value), best_id from (select *, first_value(id) over"
          + " (partition by quantum_ts order by dur desc) as best_id from %s) group by quantum_ts";
  ...
  private static final String RANGE_SQL =
      "select ts, ts + dur, value, id from %s " + "where ts + dur >= %d and ts <= %d order by ts";
  ...
  private static final String UID_SQL =
      "select uid from energy_per_uid_counter_track where consumer_id in (Select consumer_id from"
          + " energy_counter_track where id = %d)";

  private static final String UID_VALUES_SQL =
      "Select value, uid, ts, dur from (Select c1.ts + 1 ts, lead(c1.ts) over win - c1.ts dur,"
          + " lead(c1.value) over win - c1.value value, lead(uid.uid) over win uid from counter c1"
          + " inner join uid_counter_track as uid on uid.id = c1.track_id where uid.uid = %d window"
          + " win as (order by ts)) where ts + dur >= %d and ts <= %d";

  private final CounterInfo counter;

  public EnergyBreakdownTrack(QueryEngine qe, CounterInfo counter) {
    super(qe, "energy_counter_" + counter.id);
  ...
  private String viewSql() {
    return format(VIEW_SQL_MONOTONIC, counter.id);
  }

  @Override
  protected ListenableFuture<Data> computeData(DataRequest req) {
    Window win =
        (counter.count > Track.QUANTIZE_CUT_OFF) ? Window.compute(req, 5) : Window.compute(req);
  ...
  private ListenableFuture<Data> computeData(DataRequest req, Window win) {
    return transform(
        qe.query(win.quantized ? summarySql() : counterSQL()),
        res -> {
  ...
  // Updates the uidValues map with energy values corresponding to the uids received
  private ListenableFuture<Data> computerUidEnergyValues(
      Long start, Long end, Data counterData, long uid) {
    return transform(
        qe.query(uidValuesSQL(uid, start, end)),
        res -> {
          int rows = res.getNumRows();
          ...
          res.forEachRow(
              (i, r) -> {
                values[i] = r.getDouble(0);
              });
          values[rows] = values[rows - 1];
          counterData.uidValues.put(uid, values);
          return counterData;
        });
  }

  // Adda uid energy values per uid in the map under Data
  private ListenableFuture<Data> computeUids(Data counterData, long id) {
    return transform(
        qe.query(uidSQL(id)),
        res -> {
          int rows = res.getNumRows();
          ...
          counterData.hasUid = true;
          int previousRows = counterData.ts.length;
          long[] uids = new long[rows + 1];
          res.forEachRow(
              (i, r) -> {
                uids[i] = r.getLong(0);
              });
          uids[rows] = uids[rows - 1];
          counterData.uids = uids;
          return counterData;
        });
  }

  public ListenableFuture<Data> getUidEnergyValue(
      Long start, Long end, int index, long[] uids, ListenableFuture<Data> data) {
    return transformAsync(
        data,
        res -> {
          if (!res.hasUid || (index == uids.length - 1)) return data;
          ListenableFuture<Data> x = computerUidEnergyValues(start, end, res, uids[index]);
          return getUidEnergyValue(start, end, index + 1, uids, x);
        });
  }

  // Gets energy value for selected energy consumer
  public ListenableFuture<Data> getEnergyValue(long id, long counterId) {
    return transformAsync(
        expectOneRow(qe.query(valueSql(id))),
        row -> {
          ...
          return computeUids(data, counterId);
        });
  }

  // counterId -> uid value in uid_counter_table
  public ListenableFuture<Data> getValue(long id, long counterId) {
    ListenableFuture<Data> data = transform(getEnergyValue(id, counterId), res -> res);
    return transformAsync(
        data,
        res -> {
          if (res.ts.length == 0 || !res.hasUid) return data;
          return getUidEnergyValue(res.ts[0], res.ts[1], 0, res.uids, data);
        });
  }

  public ListenableFuture<Data> getValues(TimeSpan ts) {
    return transform(
        qe.query(rangeSql(ts)),
        res -> {
          int rows = res.getNumRows();
          if (rows == 0) {
  ...
  private String uidValuesSQL(long id, long start, long end) {
    return format(UID_VALUES_SQL, id, start, end);
  }

  private String uidSQL(long id) {
    return format(UID_SQL, id);
  }
  ...
  public ListenableFuture<Stats> getStats(TimeSpan span) {
    return transform(
        expectOneRow(qe.query(statsSql(span))),
  ...
  public static class Data extends Track.Data {
    ...
    public long[] uids;
    public boolean hasUid = false; // checks if the consumer has any uids available
    public Map<Long, double[]> uidValues = Maps.newHashMap(); // uid -> uid energy values
  ...
  public static class Values implements Selection<Values> {
    ...
    public Map<Long, double[]> uidValues = Maps.newHashMap(); // uid-> uid counter values

    public Values(String name, Data data) {
      this.ts = data.ts;
      this.values.put(name, data.values);
      this.ids.put(name, data.ids);
      this.uidValues = data.uidValues;
      initKeys();
    }
    ...
    public Composite buildUi(Composite parent, State state) {
      return new EnergyBreakdownSelectionView(parent, state, this);
    }
    ...
    public Values combine(Values other) {
      ...
      Map<Long, double[]> newUidValues = Maps.newHashMap();
      ...
      for (Long uid : this.uidValues.keySet()) {
        newUidValues.put(uid, new double[newTs.length]);
      }
      for (Long uid : other.uidValues.keySet()) {
        newUidValues.put(uid, new double[newTs.length]);
      }
      ...
      for (int i = 0, me = 0, them = 0; i < newTs.length; i++) {
        ...
        if (rTs == meTs) {
          ...
          for (Long uid : this.uidValues.keySet()) {
            newUidValues.get(uid)[i] = uidValues.get(uid)[me];
          }
        ...
        if (rTs == themTs) {
          ...
          for (Long uid : other.uidValues.keySet()) {
            newUidValues.get(uid)[i] = other.uidValues.get(uid)[me];
          }
          them = Math.min(them + 1, other.ts.length - 1);
        }
      }
      return new Values(newTs, newValues, newIds, newUidValues);
    }
