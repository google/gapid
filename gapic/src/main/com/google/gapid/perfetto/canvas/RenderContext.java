...
public class RenderContext implements Panel.TextMeasurer, AutoCloseable {
  ...
  public RenderContext(
      GC gc, ColorCache colors, Font icons, Panel.TextMeasurer textMeasurer, double scrollTop) {
    this.gc = gc;
    this.colors = colors;
    this.icons = icons;
  ...
  public void drawIcon(String name, double x, double y, double h) {
    Font font = gc.getFont();
    gc.setFont(icons);
    if (OS.isWindows) {
      // Workaround for bug https://bugs.eclipse.org/bugs/show_bug.cgi?id=546355
      Transform t = new Transform(gc.getDevice());
      gc.getTransform(t);
      gc.setTransform(null);
      gc.drawText(name, -20000, -20000);
      gc.setTransform(t);
      t.dispose();
    }
    ...
    try {
      // TODO: cache?
      double textH = gc.textExtent(name).y / scale;
      gc.drawText(name, scale(x), scale(y + (h - textH) / 2), SWT.DRAW_TRANSPARENT);
    } finally {
      gc.setFont(font);
    }
  }
  ...
  public static class Global implements Panel.TextMeasurer {
    ...
    public Global(Theme theme, Control owner) {
      ...
      this.icons = scaleFont(owner.getDisplay(), theme.materialIcons());
    ...
    public RenderContext newContext(GC gc, double scrollTop) {
      gc.setFont(font);
      return new RenderContext(gc, colors, icons, this, scrollTop);
    }
