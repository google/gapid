...
def output_member_dec(x, struct_name, memberid, idx, depth, vop, fdec):
    ...
    if x.name == "pNext":
        if len(x.extended_by):
            ...
            print(
                depth + f"const void** ppNext{memberid} = const_cast<const void**>(&{vop}{x.name}{idx});", file=fdec)
            print(
                depth + f"while ((pNext{memberid} = dec->decode<uint32_t>())) {{", file=fdec)
            depth = depth + "  "
            print(depth + f"switch(pNext{memberid}) {{", file=fdec)
            for y in x.extended_by:
                print(depth + f"  case {y[0]}: {{", file=fdec)
                print(
                    depth + f"    {y[1].name}* pn = dec->get_typed_memory<{y[1].name}>(1);", file=fdec)
                prms = [f"*pn", "dec"]
                prms.extend(
                    [f'bind_first(_{struct_name}{z.name()}, val)' for z in y[1].get_serialization_params(DESERIALIZE)])
                print(
                    depth + f"    deserialize_{y[1].name}(updater, {', '.join(prms)});", file=fdec)
                print(depth + f"    *ppNext{memberid} = pn;", file=fdec)
                print(
                    depth + f"    ppNext{memberid} = const_cast<const void**>(&pn->pNext);", file=fdec)
                print(depth + f"    break;", file=fdec)
                print(depth + f"  }}", file=fdec)
            if (struct_name == 'VkDeviceCreateInfo'):
                print(
                    depth + f"  case VK_STRUCTURE_TYPE_LOADER_DEVICE_CREATE_INFO:", file=fdec)
                print(depth + f"    break;", file=fdec)
            ...
            print(
                depth + 'if (dec->decode<uint32_t>()) { GAPID2_ERROR("Unexpected pNext"); }', file=fdec)
            print(depth + f'{vop}{x.name}{idx} = nullptr;', file=fdec)
    elif type(tp) == union:
        print(
            depth + f"_custom_deserialize_{tp.name}(updater, {vop}{x.name}{idx}, dec);", file=fdec)
    elif type(tp) == struct:
        prms = [f"{vop}{x.name}{idx}", "dec"]
        prms.extend(
            [f'bind_first(_{struct_name}{z.name()}, val)' for z in tp.get_serialization_params(DESERIALIZE)])
        print(
            depth + f"deserialize_{tp.name}(updater, {', '.join(prms)});", file=fdec)
    elif type(tp) == basetype:
        enc_type = tp.base_type
        if enc_type == "size_t":
            enc_type = "uint64_t"
        print(
            depth + f"dec->decode<{enc_type}>(&{vop}{x.name}{idx});", file=fdec)
        ...
        if (tp.pointee.name == "void"):
            prms = ["val", "dec"]
            print(
                depth + f"_{struct_name}_{x.name}_deserialize({', '.join(prms)});", file=fdec)
            return
        ...
        if x.len and x.len == 'null-terminated':
            ...
            # decode
            print(
                depth + f"uint64_t length{memberid} = dec->decode<uint64_t>();", file=fdec)
            print(depth + f"if (length{memberid}) {{", file=fdec)
            print(
                depth + f"    char* tmp_ = static_cast<char*>(dec->get_memory(length{memberid}));", file=fdec)
            print(
                depth + f"    dec->decode_primitive_array(tmp_, length{memberid});", file=fdec)
        ...
        if x.len:
            ...
            print(
                depth + f"uint64_t {ct} = dec->decode<uint64_t>();", file=fdec)

        tmp = f'temp{memberid}'
        print(
            depth + f"{tp.get_non_const_member(tmp)}  = dec->get_typed_memory<{tp.pointee.get_non_const_member('')[0:-1]}>({ct});", file=fdec)

        mem_idx = f'[0]'
        if x.len:
            ii = len(depth)
            print(
                depth + f"for (size_t i_{ii} = 0; i_{ii} < {ct}; ++i_{ii}) {{", file=fdec)
            depth += "  "
            mem_idx = f'[i_{ii}]'
        ...
        print(
            depth + f"dec->decode<{tp.base_type.name}>(&{vop}{x.name}{idx});", file=fdec)
    elif type(tp) == handle:
        if tp.dispatch == handle.DISPATCHABLE:
            print(
                depth + f"dec->decode<uint64_t>(reinterpret_cast<uintptr_t*>(&{vop}{x.name}{idx}));", file=fdec)
        else:
            print(
                depth + f"dec->decode<uint64_t>(reinterpret_cast<uint64_t*>(&{vop}{x.name}{idx}));", file=fdec)
    elif type(tp) == array_type:
        ii = len(depth)
        print(
            depth + f"for (size_t i_{ii} = 0; i_{ii} < {tp.size}; ++i_{ii}) {{", file=fdec)
        depth += "  "
        xm = copy.deepcopy(x)
        xm.type = tp.child
        output_member_dec(xm, struct_name, memberid + 1,
                          f"[i_{ii}]", depth, vop, fdec)
        print(depth + f"}}", file=fdec)
...
def output_member_enc(x, struct_name, memberid, idx, depth, vop, fenc):
    ...
    if x.name == "pNext":
        if len(x.extended_by):
            print(
                depth + f"auto baseStruct = reinterpret_cast<const VkBaseInStructure*>({vop}pNext);", file=fenc)
            print(depth + f"while(baseStruct) {{", file=fenc)
            print(
                depth + f"  enc->template encode<uint32_t>(baseStruct->sType);", file=fenc)
            print(depth + f"  switch(baseStruct->sType) {{", file=fenc)
            for y in x.extended_by:
                print(depth + f"    case {y[0]}:", file=fenc)
                print(
                    depth + f"      if (baseStruct->pNext != nullptr) {{", file=fenc)
                print(
                    depth + f"        {y[1].name} _tmp = *reinterpret_cast<const {y[1].name}*>(baseStruct);", file=fenc)
                print(depth + f"        _tmp.pNext = nullptr;", file=fenc)
                prms = [f"_tmp", "enc"]
                prms.extend(
                    [f'bind_first(_{struct_name}{z.name()}, val)' for z in y[1].get_serialization_params()])
                print(
                    depth + f"        serialize_{y[1].name}(updater, {', '.join(prms)});", file=fenc)
            ...
            if (struct_name == 'VkDeviceCreateInfo'):
                print(
                    depth + f"    case VK_STRUCTURE_TYPE_LOADER_DEVICE_CREATE_INFO:", file=fenc)
                print(depth + f"      break;", file=fenc)
            ...
            print(depth + f"}}", file=fenc)
            print(
                depth + f"enc->template encode<uint32_t>(0); // No more pNext", file=fenc)
            ...
            print(depth + f"  switch(baseStruct->sType) {{", file=fenc)
            print(
                depth + f"    case VK_STRUCTURE_TYPE_LOADER_INSTANCE_CREATE_INFO:", file=fenc)
            print(depth + f"      break;", file=fenc)
            print(depth + f"    default:", file=fenc)
            print(depth + f'      GAPID2_ERROR("Unexpected pNext");', file=fenc)
            print(depth + f"  }}", file=fenc)
            ...
            print(
                depth + f'if({vop}pNext) {{ GAPID2_ERROR("Unexpected pNext"); }}', file=fenc)
        ...
        print(
            depth + f"_custom_serialize_{tp.name}(updater, {', '.join(prms)});", file=fenc)
    elif type(tp) == struct:
        prms = [f"{vop}{x.name}{idx}", "enc"]
        prms.extend(
            [f'bind_first(_{struct_name}{z.name()}, val)' for z in tp.get_serialization_params()])
        print(
            depth + f"serialize_{tp.name}(updater, {', '.join(prms)});", file=fenc)
    elif type(tp) == basetype:
        enc_type = tp.base_type
        if enc_type == "size_t":
            enc_type = "uint64_t"
        print(
            depth + f"enc->template encode<{enc_type}>({vop}{x.name}{idx});", file=fenc)
    elif type(tp) == platform_type:
        enc_type = str(tp)
        if enc_type == "size_t":
            enc_type = "uint64_t"
        ...
        if (tp.pointee.name == "void"):
            prms = ["val", "enc"]
            print(
                depth + f"_{struct_name}_{x.name}_serialize({', '.join(prms)});", file=fenc)
            return
        ...
        if x.len and x.len == 'null-terminated':
            ...
            print(
                depth + f"  uint64_t len = strlen({vop}{x.name}{idx});", file=fenc)
            print(depth + f"  enc->template encode<uint64_t>(len + 1);", file=fenc)
            print(
                depth + f"  enc->template encode_primitive_array<char>({vop}{x.name}{idx}, len + 1);", file=fenc)
            print(depth + f"}} else {{", file=fenc)
        ...
        if x.noautovalidity:
            print(
                depth + f"if (_{struct_name}_{x.name}_valid(val)) {{", file=fenc)
        ...
        if x.len:
            ...
            print(
                depth + f"enc->template encode<uint64_t>({ll}); // array_len", file=fenc)
            ii = len(depth)
            print(
                depth + f"for (size_t i_{ii} = 0; i_{ii} < {ll}; ++i_{ii}) {{", file=fenc)
            depth += "  "
            mem_idx = f'{idx}[i_{ii}]'
        ...
        print(
            depth + f"enc->template encode<{tp.base_type.name}>({vop}{x.name}{idx});", file=fenc)
    elif type(tp) == handle:
        if tp.dispatch == handle.DISPATCHABLE:
            print(
                depth + f"enc->template encode<uint64_t>(reinterpret_cast<uintptr_t>({vop}{x.name}{idx}));", file=fenc)
        else:
            print(
                depth + f"enc->template encode<uint64_t>(reinterpret_cast<uint64_t>({vop}{x.name}{idx}));", file=fenc)
    elif type(tp) == array_type:
        ii = len(depth)
        print(
            depth + f"for (size_t i_{ii} = 0; i_{ii} < {tp.size}; ++i_{ii}) {{", file=fenc)
        depth += "  "
        xm = copy.deepcopy(x)
        xm.type = tp.child
        output_member_enc(xm, struct_name, memberid + 1,
                          f"[i_{ii}]", depth, vop, fenc)
        print(depth + f"}}", file=fenc)
...
def output_member_clone(x, xdst, struct_name, memberid, idx, depth, vop, dop, fclone):
    ...
    if x.name == "pNext":
        if len(x.extended_by):
            print(
                depth + f"auto srcBaseStruct = reinterpret_cast<const VkBaseOutStructure*>({vop}pNext);", file=fclone)
            ...
            for y in x.extended_by:
                ...
                print(
                    depth + f"          {y[1].name}* _val = mem->get_typed_memory<{y[1].name}>(1);", file=fclone)
                print(
                    depth + f"          if (srcBaseStruct->pNext != nullptr) {{", file=fclone)
                print(
                    depth + f"            auto srctmp_ = *reinterpret_cast<const {y[1].name}*>(srcBaseStruct);", file=fclone)
                print(depth + f"            srctmp_.pNext = nullptr;", file=fclone)
                prms = [f"srctmp_", "*_val", "mem"]
                prms.extend(
                    [f'bind_first(_{struct_name}{z.name()}, src)' for z in y[1].get_serialization_params(CLONE)])
                print(
                    depth + f"            clone<HandleUpdater>(updater, {', '.join(prms)});", file=fclone)
                print(
                    depth + f"            *dstBaseStruct = reinterpret_cast<VkBaseOutStructure*>(_val);", file=fclone)
            ...
            if (struct_name == 'VkDeviceCreateInfo'):
                print(
                    depth + f"        case VK_STRUCTURE_TYPE_LOADER_DEVICE_CREATE_INFO: {{", file=fclone)
                print(depth + f"          VkLayerDeviceCreateInfo* _val = mem->get_typed_memory<VkLayerDeviceCreateInfo>(1);", file=fclone)
                print(
                    depth + f"          memcpy(_val, srcBaseStruct, sizeof(VkLayerDeviceCreateInfo));", file=fclone)
                print(
                    depth + f"          *dstBaseStruct = reinterpret_cast<VkBaseOutStructure*>(_val);", file=fclone)
                print(depth + f"          break;", file=fclone)
                print(depth + f"        }}", file=fclone)
            ...
            print(depth + f"  switch(srcBaseStruct->sType) {{", file=fclone)
            print(
                depth + f"    case VK_STRUCTURE_TYPE_LOADER_INSTANCE_CREATE_INFO: {{", file=fclone)
            ...
            print(
                depth + f'if({vop}pNext) {{ GAPID2_ERROR("Unexpected pNext"); }}', file=fclone)
            print(depth + f"{dop}pNext = nullptr;", file=fclone)
    elif type(tp) == union:
        if x.noautovalidity:
            print(
                depth + f"if (_{struct_name}_{x.name}_valid(src)) {{", file=fclone)
            depth = depth + "  "
        prms = [f"{vop}{x.name}{idx}", f"{dop}{xdst.name}{idx}", "mem"]
        prms.extend(
            [f'bind_first(_{struct_name}{z.name()}, src)' for z in tp.get_serialization_params(CLONE)])
        print(
            depth + f"_custom_clone_{tp.name}<HandleUpdater>(updater, {', '.join(prms)});", file=fclone)
        if x.noautovalidity:
            depth = depth[:-2]
            print(depth + f"}}", file=fclone)
    elif type(tp) == struct:
        ...
        if enc_type == "size_t":
            enc_type = "uint64_t"
        print(
            depth + f"{dop}{xdst.name}{idx} = {vop}{x.name}{idx};", file=fclone)
        if x.noautovalidity:
            depth = depth[:-2]
            print(depth + f"}}", file=fclone)
    elif type(tp) == platform_type:
        ...
        if (tp.pointee.name == "void"):
            prms = ["src", "dst", "mem"]
            print(
                depth + f"_{struct_name}_{x.name}_clone({', '.join(prms)});", file=fclone)
            return
        if x.len and x.len == 'null-terminated':
            if tp.get_noncv_type().name != 'char':
                error("Expected null-terminated char list")
            ...
            print(
                depth + f"    uint64_t _len = strlen({vop}{x.name}{idx});", file=fclone)
            print(depth + f"    {{", file=fclone)
            print(
                depth + f"        char* __tmp = mem->get_typed_memory<char>(_len+1);", file=fclone)
            print(
                depth + f"        memcpy(__tmp, {vop}{x.name}{idx}, _len+1);", file=fclone)
            print(
                depth + f"        {dop}{xdst.name}{idx} = __tmp;", file=fclone)
            print(depth + f"    }}", file=fclone)
            print(depth + f"}} else {{", file=fclone)
        ...
        if x.len:
            ...
            if x.len.startswith('latexmath'):
                prms = ["val"]
                print(
                    depth + f"uint64_t _len{memberid} = _{struct_name}_{x.name}_length(src);", file=fclone)
                ct = f"_len{memberid}"

        tmp = f'temp{memberid}'
        print(
            depth + f"{tp.get_non_const_member(tmp)}  = mem->get_typed_memory<{tp.pointee.get_non_const_member('')[0:-1]}>({ct});", file=fclone)

        if x.len:
            ii = len(depth)
            print(
                depth + f"for (size_t i_{ii} = 0; i_{ii} < {ct}; ++i_{ii}) {{", file=fclone)
            depth += "  "
            mem_idx = f'{idx}[i_{ii}]'
        ...
        if x.noautovalidity:
            ...
            depth = depth + "  "
        print(
            depth + f"{dop}{xdst.name}{idx} = updater->cast_in({vop}{x.name}{idx});", file=fclone)
        if x.noautovalidity:
            depth = depth[:-2]
            print(depth + f"}}", file=fclone)
        ...
        ii = len(depth)
        print(
            depth + f"for (size_t i_{ii} = 0; i_{ii} < {tp.size}; ++i_{ii}) {{", file=fclone)
        depth += "  "
        xm = copy.deepcopy(x)
        xm.type = tp.child
        xm.noautovalidity = False
...
def output_member_fix(x, struct_name, memberid, idx, depth, vop, ffix):
    ...
    if x.name == "pNext":
        if len(x.extended_by):
            print(
                depth + f"auto srcBaseStruct = reinterpret_cast<const VkBaseOutStructure*>({vop}pNext);", file=ffix)
            ...
            print(
                depth + f'if({vop}pNext) {{ GAPID2_ERROR("Unexpected pNext"); }}', file=ffix)
    elif type(tp) == union:
        pass
    elif type(tp) == struct:
        prms = [f"{vop}{x.name}{idx}"]
        prms.extend(
            [f'bind_first(_{struct_name}{z.name()}, src)' for z in tp.get_serialization_params(FIX_HANDLES)])
        ...
        if (tp.pointee.name == "void"):
            print(
                depth + f"// _{struct_name}_{x.name} hopefully doesn't have any handles :)")
            return
        if x.len and x.len == 'null-terminated':
            if tp.get_noncv_type().name != 'char':
                error("Expected null-terminated char list")
        ...
        if x.noautovalidity:
            print(
                depth + f"if (_{struct_name}_{x.name}_valid(src)) {{", file=ffix)
            depth = depth + "  "
        elif x.optional:
        ...
        if x.len:
            ...
            if x.len.startswith('latexmath'):
                prms = ["val"]
                print(
                    depth + f"uint64_t _len{memberid} = _{struct_name}_{x.name}_length(src);", file=ffix)
                ct = f"_len{memberid}"
        ...
        if x.len:
            ii = len(depth)
            print(
                depth + f"for (size_t i_{ii} = 0; i_{ii} < {ct}; ++i_{ii}) {{", file=ffix)
            depth += "  "
            mem_idx = f'{idx}[i_{ii}]'
        ...
        output_member_fix(xm, struct_name, memberid +
                          1, mem_idx, depth, f"", ffix)
        if x.len:
            depth = depth[:-2]
            print(depth + f"}}", file=ffix)
        if x.optional or x.noautovalidity:
        ...
        print(
            depth + f"  {vop}{x.name}{idx} = cast_out({vop}{x.name}{idx});", file=ffix)
...
def output_struct_enc_dec(x, fenc, fdec):
    ...
    print(
        f"inline void serialize_{x.name}(HandleUpdater* updater, {', '.join(serialize_params)}) {{", file=fenc)

    deserialize_params = [f"{x.name}& val", "decoder* dec"]
    deserialize_params.extend([y.function()
                              for y in x.get_serialization_params(DESERIALIZE)])

    print(f"template<typename HandleUpdater>", file=fdec)
    print(
        f"inline void deserialize_{x.name}(HandleUpdater* updater, {', '.join(deserialize_params)}) {{", file=fdec)
    for y in x.members:
...
def output_struct_clone(x, fclone):
    memberid = 0
    serialize_params = ["HandleUpdater* updater",
                        f"const {x.name}& src", f"{x.name}& dst", "temporary_allocator* mem"]
    serialize_params.extend([y.function()
                            for y in x.get_serialization_params(CLONE)])
...
def output_print_member(x, struct_name, memberid, idx, depth, vop, fprint):
    ...
    if x.name == "pNext":
        if len(x.extended_by):
            ...
            print(
                depth + f"auto baseStruct = reinterpret_cast<const VkBaseInStructure*>({vop}pNext);", file=fprint)
            ...
            for y in x.extended_by:
                ...
                print(
                    depth + f"      if (baseStruct->pNext != nullptr) {{", file=fprint)
                print(
                    depth + f"        {y[1].name} _tmp = *reinterpret_cast<const {y[1].name}*>(baseStruct);", file=fprint)
                print(depth + f"        _tmp.pNext = nullptr;", file=fprint)
                prms = [f"_tmp", "printer"]
                prms.extend(
                    [f'bind_first(_{struct_name}{z.name()}, val)' for z in y[1].get_serialization_params()])
                print(
                    depth + f"        print_{y[1].name}(\"\", updater, {', '.join(prms)});", file=fprint)
            ...
            if (struct_name == 'VkDeviceCreateInfo'):
                print(
                    depth + f"    case VK_STRUCTURE_TYPE_LOADER_DEVICE_CREATE_INFO:", file=fprint)
                print(depth + f"      break;", file=fprint)
            ...
            print(
                depth + f'if({vop}pNext) {{ GAPID2_ERROR("Unexpected pNext"); }}', file=fprint)
            print(depth + f'printer->begin_array("pNext");', file=fprint)
            print(depth + f'printer->end_array();', file=fprint)
    elif type(tp) == union:
        ...
        prms.extend(
            [f'bind_first(_{struct_name}{z.name()}, val)' for z in tp.get_serialization_params()])
        print(
            depth + f"print_{tp.name}({nm}, updater, {', '.join(prms)});", file=fprint)
    elif type(tp) == basetype:
        enc_type = tp.base_type
        if enc_type == "size_t":
            enc_type = "uint64_t"
        print(
            depth + f"printer->print<{enc_type}>({nm}, {vop}{x.name}{idx});", file=fprint)
    elif type(tp) == platform_type:
        enc_type = str(tp)
        if enc_type == "size_t":
            enc_type = "uint64_t"
        ...
        if x.len and x.len == 'null-terminated':
            if tp.get_noncv_type().name != 'char':
                error("Expected null-terminated char list")
            print(
                depth + f"printer->print_string({nm}, {vop}{x.name}{idx});", file=fprint)
            return

        if x.noautovalidity:
            print(
                depth + f"if (_{struct_name}_{x.name}_valid(val)) {{", file=fprint)
            depth = depth + "  "
        elif x.optional:
        ...
        if x.len:
            ...
            ii = len(depth)
            print(
                depth + f"for (size_t i_{ii} = 0; i_{ii} < {ll}; ++i_{ii}) {{", file=fprint)
            depth += "  "
            mem_idx = f'{idx}[i_{ii}]'
        ...
        if x.noautovalidity:
            ...
            print(depth + f"}} else {{", file=fprint)
            print(
                depth + f"  printer->print<void*>({nm}, nullptr);", file=fprint)
            print(depth + f"}}", file=fprint)
        elif x.optional:
            depth = depth[:-2]
            print(depth + f"}} else {{", file=fprint)
        ...
        print(
            depth + f"printer->print<{tp.base_type.name}>({nm}, {vop}{x.name}{idx});", file=fprint)
    elif type(tp) == handle:
        if tp.dispatch == handle.DISPATCHABLE:
            print(
                depth + f"printer->print<uint64_t>({nm}, reinterpret_cast<uintptr_t>({vop}{x.name}{idx}));", file=fprint)
        else:
            print(
                depth + f"printer->print<uint64_t>({nm}, reinterpret_cast<uintptr_t>({vop}{x.name}{idx}));", file=fprint)
    elif type(tp) == array_type:
        if tp.get_noncv_type().name == 'char':
            print(
                depth + f"printer->print_char_array({nm}, {vop}{x.name}{idx}, {tp.size});", file=fprint)
            return
        print(depth + f"printer->begin_array({nm});", file=fprint)
        ii = len(depth)
        print(
            depth + f"for (size_t i_{ii} = 0; i_{ii} < {tp.size}; ++i_{ii}) {{", file=fprint)
        depth += "  "
        xm = copy.deepcopy(x)
        xm.type = tp.child
        output_print_member(xm, struct_name, memberid + 1,
                            f"[i_{ii}]", depth, vop, fprint)
        depth = depth[:-2]
        print(depth + f"}}", file=fprint)
        print(depth + f"printer->end_array();", file=fprint)
...
def output_print_struct(x, fprint):
    ...
    print(
        f"inline void print_{x.name}(const char* name, HandleUpdater* updater, {', '.join(serialize_params)}) {{", file=fprint)
...
def output_arg_create(cmd, param, file):
    ...
    if type(tp) == pointer_type:
        ...
        if cmd.name == "vkCreateInstance":
            ...
            print(
                f"    create_instance<HandleUpdater, {pt}Wrapper<HandleUpdater>>({', '.join(prms)});", file=file)
            return
        ...
        if type(tp.pointee) == struct:
            print(
                f"    create_handle_from_struct<HandleUpdater>({', '.join(prms)});", file=file)
        else:
            print(
                f"    create_handle<HandleUpdater, {zn}, {pt}, {pt}Wrapper<HandleUpdater>>({', '.join(prms)});", file=file)
...
def output_arg_enc(cmd, x, vop, arg_idx, idx, depth, fenc):
    ...
    if type(tp) == struct:
        ...
        if (tp.pointee.name == "void"):
            ...
            prms.append("enc")
            print(
                depth + f"_custom_serialize_{cmd.name}_{x.name}(updater, {', '.join(prms)});", file=fenc)
            return
        # Special case for strings
        ...
        if x.len:
            ...
            if len(p):
                if type(p[0].type) == pointer_type:
                    argct = f"*{argct}"
            print(
                depth + f"for (size_t i_{ii} = 0; i_{ii} < {argct}; ++i_{ii}) {{", file=fenc)
            depth += "  "
            mem_idx = f'{idx}[i_{ii}]'
...
def output_command(cmd, definition, fbod, only_return=False):
    ...
    if (cmd.name == "vkWaitSemaphoresKHR" or cmd.name == "vkWaitSemaphores"):
        print(
            f"    auto enc = get_encoder(reinterpret_cast<uintptr_t>({cmd.args[0].name}));", file=fbod)
    else:
        print(
            f"    auto enc = get_locked_encoder(reinterpret_cast<uintptr_t>({cmd.args[0].name}));", file=fbod)
    ...
    if not only_return:
        ...
        for arg in cmd.args:
            if arg.name == 'pAllocator':
                print(
                    f"      // Skipping: {arg.name} for as it cannot be replayed", file=fbod)
                continue
            if type(arg.type) == pointer_type and not arg.type.const:
                if arg.inout():
                    ...
                    print(
                        f"      // Skipping: {arg.name} for now as it is an output param", file=fbod)
                    continue
...
''', file=fbod)
            for cmd in definition.commands.values():
                ...
                print(
                    f"      return (PFN_vkVoidFunction) {cmd.name};", file=fbod)
                print(f"    }}", file=fbod)
            print(
                '''
    return nullptr;
...
''', file=fbod)
        for cmd in definition.commands.values():
            ...
            if cmd.args[0].name != 'device' and cmd.args[0].name != 'commandBuffer' and cmd.args[0].name != 'queue' and cmd.args[0].name != 'instance' and cmd.args[0].name != 'physicalDevice':
                ...
                print(
                    f"    const auto fn = updater->cast_from_vk({cmd.args[0].name})->_functions->{cmd.name};", file=fbod)
...
def print_arg_ptr(cmd, param, tp, count, depth, fprint):
    ...
    if tp.name == "void":
        ...
        print(
            depth + f"printer->print<{tp.base_type.name}>(\"{nm}\", {param.name}{idx});", file=fprint)
...
def output_arg_print(cmd, param, depth, fprint):
    ...
    if type(tp) == enum:
        ...
        if param.len and param.len == 'null-terminated':
            ...
            print(
                depth + f"printer->print_string(\"{nm}\", {nm});", file=fprint)
            return
        if param.optional:
    ...
    with open(os.path.join(dir, "forwards.h"), mode="w") as fhead:
        ...
        for cmd in definition.commands.values():
            ...
            for arg in cmd.args:
                ...
                all_enc_args.extend(
                    [y.signature() for y in tp.get_serialization_params(DESERIALIZE)])
                all_enc_args.extend([y.signature()
                                    for y in tp.get_serialization_params(CLONE)])
...
def output_device_functions(definition, dir, commands_to_handle=None):
    with open(os.path.join(dir, "device_functions.h"), mode="w") as fhead:
        ...
        print(
            '        DeviceFunctions(VkDevice device, PFN_vkGetDeviceProcAddr get_device_proc_addr) {', file=fhead)
        for cmd in definition.commands.values():
            if commands_to_handle:
                if not cmd in commands_to_handle:
                    continue
            if cmd.args[0].name != 'device' and cmd.args[0].name != 'commandBuffer' and cmd.args[0].name != 'queue':
                continue
            print(
                f'            {cmd.name} = reinterpret_cast<PFN_{cmd.name}>(get_device_proc_addr(device, "{cmd.name}"));', file=fhead)
        print('        };', file=fhead)
        for cmd in definition.commands.values():
            if commands_to_handle:
                if not cmd in commands_to_handle:
...
def output_instance_functions(definition, dir, commands_to_handle=None):
    with open(os.path.join(dir, "instance_functions.h"), mode="w") as fhead:
        ...
        print(
            '        InstanceFunctions(VkInstance instance, PFN_vkGetInstanceProcAddr get_instance_proc_addr) {', file=fhead)
        for cmd in definition.commands.values():
            if commands_to_handle:
                if not cmd in commands_to_handle:
                    continue
            if cmd.args[0].name != 'instance' and cmd.args[0].name != 'physicalDevice':
                continue
            print(
                f'            {cmd.name} = reinterpret_cast<PFN_{cmd.name}>(get_instance_proc_addr(instance, "{cmd.name}"));', file=fhead)
        print('        };', file=fhead)
        for cmd in definition.commands.values():
            if commands_to_handle:
                if not cmd in commands_to_handle:
...
def output_arg_dec(cmd, x, vop, arg_idx, idx, depth, fenc):
    ...
    if type(tp) == union:
        ...
        print(
            depth + f"_custom_deserialize_{tp.name}(updater, {', '.join(prms)});", file=fenc)
    if type(tp) == struct:
        prms = [f"{vop}{x.name}{idx}", "decoder_"]
        prms.extend([x.name()
                    for x in tp.get_serialization_params(DESERIALIZE)])
        print(
            depth + f"deserialize_{tp.name}(updater, {', '.join(prms)});", file=fenc)
    elif type(tp) == basetype:
        if tp.base_type == "size_t":
            print(
                depth + f"{vop}{x.name}{idx} = static_cast<size_t>(decoder_->decode<uint64_t>());", file=fenc)
        else:
            print(
                depth + f"{vop}{x.name}{idx} = decoder_->decode<{tp.base_type}>();", file=fenc)
    elif type(tp) == platform_type:
        if str(tp) == "size_t":
            print(
                depth + f"{vop}{x.name}{idx} = static_cast<size_t>(decoder_->decode<uint64_t>());", file=fenc)
            ...
            print(
                depth + f"{vop}{x.name}{idx} = decoder_->decode<{str(tp)}>();", file=fenc)
    elif type(tp) == pointer_type:
        if (tp.pointee.name == "void"):
            prms = [x.name for x in cmd.args]
            prms.append("decoder_")
            print(
                depth + f"_custom_deserialize_{cmd.name}_{x.name}(updater, {', '.join(prms)});", file=fenc)
            return
        # Special case for strings
        if x.len and x.len.startswith('latexmath'):
            print(depth + f"// Latexmath string", file=fenc)
        ...
        if x.len and x.len == 'null-terminated':
            ...
            print(
                depth + f"  {vop}{x.name}{idx} = static_cast<{x.type.get_noncv_type().get_member('')[0:-1]}*>(decoder_->get_memory(len_));", file=fenc)
            print(
                depth + f"  decoder_->decode_primitive_array<char>({vop}{x.name}{idx}, len_);", file=fenc)
            print(depth + f"}} else {{", file=fenc)
        ...
        if x.optional or x.len:
            print(
                depth + f"{vop}{x.name}{idx} = decoder_->get_typed_memory<{tp.pointee.get_non_const_member('')[0:-1]}>({ct});", file=fenc)
        ...
        print(
            depth + f"{vop}{x.name}{idx} = static_cast<{tp.name}>(decoder_->decode<{tp.base_type.name}>());", file=fenc)
    elif type(tp) == handle:
        if tp.dispatch == handle.DISPATCHABLE:
            print(
                depth + f"{vop}{x.name}{idx} = reinterpret_cast<{tp.name}>(static_cast<uintptr_t>(decoder_->decode<uint64_t>()));", file=fenc)
        else:
            print(
                depth + f"{vop}{x.name}{idx} = reinterpret_cast<{tp.name}>(decoder_->decode<uint64_t>());", file=fenc)
    elif type(tp) == array_type:
        print(depth + f"for (size_t i = 0; i < {tp.size}; ++i) {{", file=fenc)
        depth = depth + "  "
        mem_idx = f'{idx}[i]'
...
def output_arg_register(cmd, param, file):
    ...
    if type(tp) == pointer_type:
        ...
        if type(tp.pointee) == struct:
            prms = [param.name, len]
            print(
                f"    this->updater->register_handle_from_struct({', '.join(prms)});", file=file)
        else:
            prms = [param.name, len]
            print(
                f"    this->updater->register_handle({', '.join(prms)});", file=file)
...
def output_command_deserializer(cmd, definition, fbod):
    ...
    for arg in cmd.args:
        ...
        if type(t) == array_type:
            print(
                f"    {t.child.get_noncv_type().name} {arg.name}[{t.size}];", file=fbod)
            continue
        ...
        if not arg.optional and not arg.len:
            print(
                f"    {t.pointee.get_noncv_type().name} {name}[1];", file=fbod)
        elif arg.optional:
            # Is a pointer type
            print(
                f"    {arg.type.get_noncv_type().name}* {name}; // optional", file=fbod)
        elif arg.len:
            print(
                f"    {arg.type.get_noncv_type().name}* {name}; // length {arg.len}", file=fbod)
    ...
    for arg in cmd.args:
        ...
        if arg.inout():
            if not arg.optional and not arg.len:
                print(
                    f"    {arg.type.pointee.name} {arg.name}[1]; // inout", file=fbod)
            elif arg.optional:
                # Is a pointer type
                print(
                    f"    {arg.type.name} {arg.name}; // optional inout", file=fbod)
            elif arg.len:
                print(
                    f"    {arg.type.name} {arg.name}; // length {arg.len} inout", file=fbod)
            ...
            if arg.len:
                ...
                if len(p):
                    if type(p[0].type) == pointer_type:
                        ct = f"*{ct}"
            print(
                f"    memcpy({arg.name}, tmp_{arg.name}, sizeof({arg.name}[0]) * {ct}); // setting inout properly", file=fbod)
...
''', file=fbod)
        ...
        print(
            "  // A raw stream does not have command lengths encoded inside it.", file=fbod)
        print(
            "  void DeserializeStream(decoder* decoder_, bool raw_stream = false) {", file=fbod)
        print("    do {", file=fbod)
        print("      if (!raw_stream) {", file=fbod)
        print("        const uint64_t data_left = decoder_->data_left();", file=fbod)
        print(
            "        if (data_left < sizeof(uint64_t)) { return; }", file=fbod)
        print(
            "        if (data_left - sizeof(uint64_t) < decoder_->decode<uint64_t>()) { return; } ", file=fbod)
        print("      } else {", file=fbod)
        print(
            "        if (!decoder_->has_data_left()) { return; } ", file=fbod)
        print("      }", file=fbod)
        ...
        for cmd in definition.commands.values():
            ...
            sha = int.from_bytes(hashlib.sha256(
                cmd.name.encode('utf-8')).digest()[:8], 'little')
            print(
                f"        case {sha}u: {cmd.name}(decoder_); continue;", file=fbod)
        print('''
        default:
            std::abort();
...
def output_forward_header(definition, dir, commands_to_handle=None):
    with open(os.path.join(dir, "layer_internal.inl"), mode="w") as fcall:
        ...
        for cmd in definition.commands.values():
            ...
            print(
                f'{cmd.ret.short_str()} (*call_{cmd.name})(void*, {", ".join(prms)});', file=fcall)

        for x in definition.types.values():
            if type(x) == handle:
                print(
                    f'{x.name} (*get_raw_handle_{x.name})(void* data_, {x.name} in);', file=fcall)
        print(
            '''
extern "C" {
...
''', file=fcall)
        ...
        for cmd in definition.commands.values():
            if commands_to_handle:
                if not cmd in commands_to_handle:
                    continue
            print(
                f'            f.fn_{cmd.name} = &next_layer_{cmd.name};', file=fcall)
            print(
                f'            f.{cmd.name}_user_data = static_cast<T*>(this);', file=fcall)
        print('''
        ...
        for cmd in definition.commands.values():
            ...
            print(
                f'              *fout = this_->f.{cmd.name}_user_data;', file=fcall)
            print(
                f'              return reinterpret_cast<void*>(this_->f.fn_{cmd.name});', file=fcall)
            print(f'            }}', file=fcall)
        print('''
        ...
        for x in definition.types.values():
            if type(x) == handle:
                print(
                    f'            if (!strcmp(tp, "{x.name}")) {{', file=fcall)
                print(
                    f'              auto r = Layerer<T, HandleUpdater>::get_raw_handle<{x.name}>;', file=fcall)
        ...
        for cmd in definition.commands.values():
            ...
            print(
                f'            auto f_{cmd.name} = ({cmd.ret.short_str()}(*)({", ".join(prms)}))GetProcAddress(lib, "override_{cmd.name}");', file=fcall)
            print(f'            if (f_{cmd.name}) {{', file=fcall)
            print(
                f'              f.{cmd.name}_user_data = f_{cmd.name};', file=fcall)
            print(
                f'              f.fn_{cmd.name} = &forward_{cmd.name};', file=fcall)
            print(
                f'              std::cerr << "Found function override_{cmd.name} in layer, setting up chain" << std::endl;', file=fcall)
            print(f'            }}', file=fcall)
        print(
            '''
          RunUserSetup(lib);
...
''', file=fbod)
        for cmd in definition.commands.values():
            ...
            if (cmd.ret.name == 'void'):
                ...
                print(
                    f"    const auto ret = fn(f.{cmd.name}_user_data, {args});", file=fbod)
...
def output_indirect_functions(definition, dir, commands_to_handle=None):
    with open(os.path.join(dir, "fns.h"), mode="w") as fns:
        ...
        for cmd in definition.commands.values():
            ...
            prms = [x.short_str() for x in cmd.args]
            print(
                f'      {cmd.ret.short_str()}(*fn_{cmd.name})(void*, {", ".join(prms)});', file=fns)
...
def output_command_buffer_recorder_fns(definition, dir, commands_to_handle=None):
    with open(os.path.join(dir, "command_buffer_recorder.inl"), mode="w") as fns:
        ...
        for cmd in definition.commands.values():
            ...
            print(
                f'inline {cmd.ret.short_str()} forward_cmd_{cmd.name}(void* f, {", ".join(prms)}) {{', file=fns)
            print(
                f'  return reinterpret_cast<CommandBufferRecorder<T>*>(f)->{cmd.name}({", ".join([x.name for x in cmd.args])});', file=fns)
            print(f'}}', file=fns)

        print('''
template<typename T>
void CommandBufferRecorder<T>::SetupCommandBuf  ferRecorderFunctions(fns& f) {
...
''', file=fns)
        for cmd in definition.commands.values():
            ...
            if cmd.args[0].name != "commandBuffer":
                continue
            print(
                f'    f.fn_{cmd.name} = forward_cmd_{cmd.name}<T>;', file=fns)
...
def main():
    parser = argparse.ArgumentParser(
        description='Processes the vulkan XML into code')
