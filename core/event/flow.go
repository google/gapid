...
func Filter(ctx log.Context, pred Predicate, handler Handler) Handler {
...
func FilterAny(ctx log.Context, pred interface{}, dst interface{}) Handler {
...
func Feed(ctx log.Context, dst Handler, src Producer) error {
	for {
		event := src(ctx)
		if event == nil {
			return nil
...
// done.
func Connect(ctx log.Context, listener Listener, handler Handler) error {
...
func Drain(ctx log.Context, handler Handler) (Handler, <-chan error) {
...
func Monitor(ctx log.Context, lock sync.Locker, listener Listener, producer Producer, handler Handler) error {
	var done <-chan error
